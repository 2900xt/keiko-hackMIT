...
Runs on whatever CONFIG_KEIKO_UDP_HOST in keiko.conf points at (any machine on the subnet when it is
255.255.255.255). `make listen` / `make record` wrap it. Standard library only. Header format: see
src/main.c (identical to firmware/unoq/python/main.py's).
"""
import argparse
import socket
import struct
        ...
        if data and len(data) >= HDR.size:
            ...
            if magic == b"KEIK":
                ...
                if a.wav and fs > 0:
                    ...
                    m = sum(x) / n
                    dc = m if dc is None else 0.98 * dc + 0.02 * m          # same tracker as the node's health line
                    if wav is None:
