...
Built 2026-09-19. **The 16 GB of audio is not in git.** Run `./fetch_data.sh` (needs `aria2c`, `unzip`, `python3` with
`huggingface_hub pyarrow soundfile`) to download the raw sources (~20 GB) and rebuild `audio/` exactly as indexed in
...
| `audio/<dataset>/<taxon or label>/*.wav` | 108,246 extracted clips (16 GB). Every file has a row in `clips`. |
...
| Cornell/Marinexplore Whale Detection Challenge (Kaggle 2013) via the timeseriesclassification.com `RightWhaleCalls` mirror | 12,896 | 7.2 | North Atlantic right whale *Eubalaena glacialis* upcalls (6,448) vs noise (6,448), Cornell MARU buoys, Massachusetts Bay | binary, 2 s @ 2 kHz; train/test split preserved in `source_record_id` | Kaggle competition rules (research) |
...
Totals: **108,246 clips, ~78 hours, 59 species with audio** (30 toothed whales, 8 baleen whales, 14 pinnipeds,
6 fish, 1 sea otter) plus family-level and unidentified classes. Species with the most audio: oyster toadfish
(10k), North Atlantic right whale (6.9k), plainfin midshipman (4.7k), killer whale (3.5k), humpback (3.1k), sperm whale (1.3k, 11.7 h).
...
- `datasets`: 75 datasets from the papers (DCLDE 2013–2027, NOAA NEFSC/PIFSC/SanctSound, Antarctic blue/fin
  ...
  label format, license and the papers that used them. `downloaded=1` marks the eight with audio here.
...
- Right whale clips are 2 kHz and float-derived; a few peak at full scale. Class imbalance elsewhere is severe. Toadfish and midshipman dominate fish; common dolphin and false killer whale dominate Watkins.
...
ecotype set (225k boxes, 1.6 TB), Antarctic blue/fin library (105k annotations), BEANS `hiceas` minke boings (1.4 GB), the full 30k-clip Kaggle right-whale training set (needs a Kaggle login; the mirror here is the 12.9k balanced subset),
